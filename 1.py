            ...
            for i in range(num_lanes):
                ...
                if sensor_now == 0 and last_sensor_state[i] == 1:
                    if (now - last_sensor_trigger_time[i]) > debounce_time:
                        ...
                        # ============================================
                        # (NÂNG CẤP) LOGIC XỬ LÝ SENSOR MỚI
                        # ============================================
                        with queue_lock:
                            if i in qr_queue:
                                # --- 1. KHỚP HÀNG CHỜ (QR tới trước) ---
                                qr_queue.remove(i) # Dùng remove() cho phép xử lý linh hoạt (không cần FIFO)
                                current_queue_for_log = list(qr_queue)
                                queue_head_since = now if qr_queue else 0.0
                                ...
                                with state_lock:
                                    if 0 <= i < len(system_state["lanes"]):
                                        ...
                                        # (SỬA LỖI) Đặt trạng thái chờ đẩy cho lane CÓ PISTON
                                        if push_pin is None:
                                ...
                                # (SỬA LỖI) Khởi chạy luồng xử lý cho TẤT CẢ các lane (cả đẩy và thẳng)
                                threading.Thread(target=handle_sorting_with_delay, args=(i,), daemon=True).start()

                                broadcast_log({
                                    "log_type": "info",
                                    "message": f"Sensor {lane_name_for_log} khớp hàng chờ.",
                                    "queue": current_queue_for_log
                                })
                                
                                # Xóa các cờ chờ sensor khác (nếu có)
                                if 0 <= i < len(pending_sensor_triggers):
                                    for j in range(len(pending_sensor_triggers)):
                                        if i != j:
                                            pending_sensor_triggers[j] = 0.0

                            elif not qr_queue:
                                # --- 2. HÀNG CHỜ RỖNG (Sensor tới trước) ---
                                
                                # (NÂNG CẤP) Đặt cờ chờ QR cho lane này
                                if 0 <= i < len(pending_sensor_triggers):
                                ...
                                # Chỉ xử lý (đếm) ngay cho lane đi thẳng (không QR)
                                if push_pin is None:
                                ...
                                # Nếu là lane đẩy, nó sẽ chờ QR trong luồng qr_detection_loop

                            else:
                                # --- 3. LỖI THỨ TỰ (Queue không rỗng, nhưng không chứa i) ---
                                current_queue_for_log = list(qr_queue)
                                broadcast_log({
                                    "log_type": "error",
                                    "message": f"LỖI THỨ TỰ: Sensor {lane_name_for_log} kích hoạt nhưng không có trong hàng chờ. (Hàng chờ: {current_queue_for_log})",
                                    "queue": current_queue_for_log
                                })
                                
                                # (SỬA LỖI) KHÔNG KÍCH HOẠT BẢO TRÌ
                                # error_manager.trigger_maintenance(...) # <-- ĐÃ XÓA DÒNG NÀY

                                # Chỉ xử lý (đếm) cho lane đi thẳng
                                if push_pin is None:
                                    broadcast_log({"log_type": "info", "message": f"Vật lạ/lỗi thứ tự đi thẳng qua {lane_name_for_log}."})
                                    threading.Thread(target=sorting_process, args=(i,), daemon=True).start()
                                # Nếu là lane đẩy, bỏ qua (để vật đi thẳng)
