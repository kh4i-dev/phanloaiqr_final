# -*- coding: utf-8 -*-
"""
BẢN NÂNG CẤP APP.PY (vLogic 3.1 - Giữ Mock/Auth ẩn)

Phiên bản này giữ nguyên cấu trúc đầy đủ của app.py (Mock, Auth, ErrorManager)
nhưng được tích hợp các logic "xịn" mới:

- TÍNH NĂNG GỐC (ẨN):
    - MockGPIO: Vẫn tồn tại, tự kích hoạt nếu không thấy RPi.GPIO.
    - Auth: Vẫn tồn tại, nhưng TẮT theo mặc định (AUTH_ENABLED=false).
- SỬA LOGIC FIFO: Chuyển sang logic 'if i in qr_queue:' (FIFO linh hoạt)
  để xử lý vật phẩm bị vượt hàng.
- SỬA LOGIC TEST:
    - Test đơn (nhấn nút) sẽ GIỮ NGUYÊN trạng thái (Đẩy hoặc Thu).
    - Test tuần tự sẽ chạy ĐẦY ĐỦ chu trình (Đẩy -> Thu) cho từng lane.
- TÍNH NĂNG MỚI: Tải 'queue_head_timeout' và 'pending_trigger_timeout'
  từ config.json.
"""
import cv2
import time
...
# =============================
hot_reload_enabled = False  # Giữ nguyên
...
# (Giữ nguyên) Auth được TẮT theo mặc định
...
executor = ThreadPoolExecutor(max_workers=3, thread_name_prefix="TestWorker") # Giữ nguyên Executor
...
def load_local_config():
    ...
    global QUEUE_HEAD_TIMEOUT, PENDING_TRIGGER_TIMEOUT # (MỚI)

    # (MỚI) Cập nhật default timing với các giá trị timeout
    default_timing_config = {
        "cycle_delay": 0.3, "settle_delay": 0.2, "sensor_debounce": 0.1,
        "push_delay": 0.0, "gpio_mode": "BCM",
        "queue_head_timeout": 15.0, # Mặc định 15s
        "pending_trigger_timeout": 0.5 # Mặc định 0.5s
    }
    default_config_full = {
        ...
        try:
            with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                ...
                if not file_content:
                    ...
                    # Merge timing config (ưu tiên file)
                    ...
                    # Merge lanes config (ưu tiên file)
    ...
    for i, lane_cfg in enumerate(lanes_config):
        ...
        lane_id = lane_cfg.get("id", f"LANE_{i+1}")
    ...
    QUEUE_HEAD_TIMEOUT = system_state['timing_config'].get('queue_head_timeout', 15.0)
    PENDING_TRIGGER_TIMEOUT = system_state['timing_config'].get('pending_trigger_timeout', 0.5)
...
def reset_all_relays_to_default():
    """(Giữ nguyên) Reset tất cả relay về trạng thái an toàn (THU BẬT, ĐẨY TẮT)."""
...
def periodic_config_save():
    """(Giữ nguyên) Tự động lưu config mỗi 60s."""
...
def log_sort_count(lane_index, lane_name):
    """(Giữ nguyên) Ghi lại số lượng đếm vào file JSON."""
    with sort_log_lock:
        try:
            ...
            if os.path.exists(SORT_LOG_FILE):
                try:
                    ...
                    logging.error(f"[SORT_LOG] Lỗi đọc {SORT_LOG_FILE}, file có thể bị hỏng: {e}")
                    sort_log = {}
...
def sorting_process(lane_index):
    """(Giữ nguyên) Quy trình đẩy-thu piston."""
    lane_name = ""
...
def handle_sorting_with_delay(lane_index):
    """(Giữ nguyên) Luồng trung gian, chờ push_delay."""
    push_delay = 0.0
    ...
    try:
        ...
        if current_status in ["Đang chờ đẩy", "Sẵn sàng"]:
            sorting_process(lane_index) # (SỬA) Bỏ executor, chạy trực tiếp
        else:
...
def _run_test_relay(lane_index, relay_action):
    ...
    try:
        ...
        # Không sleep, không tự động OFF. Giữ nguyên trạng thái.
        ...
        reset_all_relays_to_default() # Reset về an toàn nếu lỗi
...
def _run_test_all_relays():
    ...
    try:
        ...
        for i in range(num_lanes):
            ...
            RELAY_ON(push_pin); time.sleep(cycle_delay)
            if not main_loop_running or not test_seq_running: stopped_early = True; break
        ...
        reset_all_relays_to_default() # Luôn reset về an toàn
...
def qr_detection_loop():
    ...
    while main_loop_running:
        try: # (MỚI) Thêm try...except để luồng không bị crash
            ...
            if data and (data != last_qr or time.time() - last_time > 3.0):
                ...
                if data_key in LANE_MAP:
                    ...
                    if is_pending_match:
                        ...
                        system_state["queue_indices"] = current_queue_for_log # Cập nhật state
            ...
            time.sleep(0.5) # Nghỉ 0.5s nếu có lỗi
...
def sensor_monitoring_thread():
    ...
    try:
        while main_loop_running:
            ...
            # (MỚI) Đọc các giá trị timeout từ config (để có thể thay đổi động)
            with state_lock:
            ...
            with flexible_fifo_lock: # (SỬA) Dùng lock mới
                if flexible_fifo_queue and queue_head_since > 0.0:
                    if (now - queue_head_since) > QUEUE_HEAD_TIMEOUT:
                        expected_lane_index = flexible_fifo_queue[0] # Lấy vật đầu hàng
                        ...
                        flexible_fifo_queue.pop(0) # Xóa vật đầu hàng
                        current_queue_for_log = list(flexible_fifo_queue)
            ...
            for i in range(num_lanes):
                ...
                try:
                    ...
                    logging.error(f"[SENSOR] Lỗi đọc GPIO pin {sensor_pin} (Lane {lane_name_for_log}): {gpio_e}")
                    error_manager.trigger_maintenance(f"Lỗi đọc sensor pin {sensor_pin} (Lane {lane_name_for_log}): {gpio_e}")
                    continue
                    ...
                    if (now - last_sensor_trigger_time[i]) > debounce_time:
                        ...
                        with flexible_fifo_lock: # (SỬA) Dùng lock mới
                            if i in flexible_fifo_queue:
                                ...
                                is_head = (i == flexible_fifo_queue[0]) # Kiểm tra xem có phải đầu hàng không
                                ...
                                if push_pin is None: # Lane đi thẳng
                                    ...
                                    # (SỬA) Bỏ executor, chạy sorting_process (vì không cần delay)
...
def requires_auth(f):
    """Decorator để yêu cầu đăng nhập (nếu AUTH_ENABLED=true)."""
    @functools.wraps(f)
...
@requires_auth
def index():
    # (SỬA) Đảm bảo render index.html (bản full)
...
def update_config():
    ...
    with state_lock:
        ...
        # (MỚI) Cập nhật đầy đủ, bao gồm cả các timeout
        current_timing.update(new_timing_config)
        
        # (MỚI) Cập nhật các biến global timeout
        ...
        if new_lanes_config is not None:
            ...
            for lane_state in system_state['lanes']:
                current_lanes_cfg_for_save.append({
                    "id": lane_state.get('id'), "name": lane_state['name'],
                    "sensor_pin": lane_state.get('sensor_pin'),
                    "push_pin": lane_state.get('push_pin'),
                    "pull_pin": lane_state.get('pull_pin')
                })
    ...
    try:
        ...
        msg = "Đã lưu config. "
        if restart_required:
             msg += "Vui lòng khởi động lại hệ thống để áp dụng thay đổi."
        else:
             msg += "Các thay đổi về timing đã được áp dụng."
...
def reset_maintenance():
    """(Giữ nguyên) API (POST) để reset chế độ bảo trì."""
    global pending_sensor_triggers, queue_head_since
    if error_manager.is_maintenance():
        error_manager.reset()
        with flexible_fifo_lock: # (SỬA) Dùng lock mới
...
@requires_auth
def api_queue_reset():
    """(Giữ nguyên) API (POST) để xóa hàng chờ QR."""
    global pending_sensor_triggers, queue_head_since
    if error_manager.is_maintenance():
...
@requires_auth
def api_mock_gpio():
    """(Giữ nguyên) API (POST) để điều khiển sensor ở chế độ giả lập."""
...
# =============================
@sock.route('/ws')
@requires_auth # (Giữ nguyên)
def ws_route(ws):
    """Kết nối WebSocket chính."""
    global AUTO_TEST_ENABLED

    # (Giữ nguyên) Logic kiểm tra auth
    auth_user = "guest"
    if AUTH_ENABLED:
        auth = request.authorization
        if not auth or not check_auth(auth.username, auth.password):
...
if __name__ == "__main__":
    try:
        ...
        logging.info("=========================================")
        logging.info("  HỆ THỐNG PHÂN LOẠI SẴN SÀNG (vAPP-PRO / Logic 3.1)")
